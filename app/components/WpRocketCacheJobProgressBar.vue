...
<template>
  ...
  >
    <div class="px-2 pt-2 pb-3 flex items-center gap-3">
      ...
      />
      <div v-if="!collapsed" class="text-sm font-medium flex-1">
        {{ statusText }}
        <div v-if="wpRocketCacheJobStore.progress" class="text-xs text-neutral-500">
          {{ wpRocketCacheJobStore.progress.success }} success, {{ wpRocketCacheJobStore.progress.failed }} failed, {{ wpRocketCacheJobStore.progress.skipped }} skipped
        </div>
      </div>
    </div>
    <div v-if="wpRocketCacheJobStore.isRunning && !collapsed" class="px-2 w-full pt-2 pb-4">
      <UProgress
