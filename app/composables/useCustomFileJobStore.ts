...
export interface CustomFileJobLogEntry {
  timestamp: Date
...
export interface CustomFileJobProgress {
  runId: string | null
  total: number
  current: number
  queued: number
...
export const useCustomFileJobStore = defineStore('custom-file-job', () => {
  const activeProgress = ref<CustomFileJobProgress | null>(null)
  const lastCompletedProgress = ref<CustomFileJobProgress | null>(null)
  const logs = ref<CustomFileJobLogEntry[]>([])
  ...
  const connectWebSocket = () => {
    ...
    ws.value.onmessage = (event) => {
      try {
        ...
        if (data?.channel !== 'custom-file-job') return
        ...
        logs.value.push({
          ...
          customFileId: data?.data?.job?.customFileId,
          originalFilename: data?.data?.job?.originalFilename
        })
        ...
        console.error('Failed to parse custom file job event:', error)
      }
    }
    ...
    ws.value.onerror = (error) => {
      console.error('Custom file job WebSocket error:', error)
      wsConnected.value = false
    }
  }
  ...
  const refreshStatus = async () => {
    try {
      const snapshot = await useApiClient()<CustomFileJobProgress>('/custom-files/jobs/status')
      ...
      console.error('Failed to fetch custom file queue status:', error)
    }
  }
...
function normalizeProgress(input: any): CustomFileJobProgress | null {
