...
type ScanInstallationContext = {
  serverId: string
  serverType: 'PLESK' | 'CPANEL_WHM'
  phpBinary: string
  defaultNewSiteLevel: MonitoringLevel
  knownInstallationPaths: Set<string>
  hadCompletedScan: boolean
  newSitesFound: NewSiteReportEntry[]
 ...
 */
async function setupWpCliAndDetectPhp(
  ssh: SshSession,
  server: Pick<ServerForScan, 'serverType'>,
  sendEvent: (type: ScanEventType, message: string, data?: any) => void
): Promise<string> {
...
/**
 * Scans a single WordPress installation at `wpDir`, upserting it and its
 * plugins/themes into the database. Returns true on success, false on failure.
 */
async function scanInstallation(
  wpDir: string,
  session: SshSession,
  ctx: ScanInstallationContext
): Promise<boolean> {
  const { sendEvent, phpBinary } = ctx
  try {
    sendEvent('log', `Processing: ${wpDir}`)

    let unixUser: string
    if (ctx.serverType === 'CPANEL_WHM') {
    ...
    const isNewInstallation = !ctx.knownInstallationPaths.has(wpDir)
    ...
    ctx.knownInstallationPaths.add(wpDir)

    if (ctx.hadCompletedScan && isNewInstallation) {
      ctx.newSitesFound.push({
        siteTitle,
        siteUrl,
        installationPath: wpDir
      })
...
export async function runServerScan(server: ServerForScan): Promise<ServerScanResult> {
  ...
  try {
    ...
    const scanContext: ScanInstallationContext = {
      serverId: server.id,
      serverType: server.serverType,
      phpBinary,
      defaultNewSiteLevel,
      knownInstallationPaths,
      hadCompletedScan,
      newSitesFound,
      sendEvent
    }

    const processInstallation = async (wpDir: string, session: SshSession) => {
      const success = await scanInstallation(wpDir, session, scanContext)
      if (success) {
        successCount++
      } else {
        failedCount++
...
/**
 * Re-scans a single, already-known WordPress installation. Reuses the
 * server-scoped scan events (keyed by serverId) so the existing scan progress
 * UI reflects the activity.
 */
export async function runSiteScanById(siteId: string): Promise<ServerScanResult> {
  ...
  const wpDir = installation.installationPath
  const sendEvent = (type: ScanEventType, message: string, data?: any) =>
    sendScanEvent(server.id, type, message, data)
  ...
  sendEvent('progress', 'Starting site scan', { total: 1, current: 0, success: 0, failed: 0 })
  ...
  try {
    ...
    sendEvent('log', `Re-scanning ${wpDir} on ${server.name}...`)
    ...
    const scanContext: ScanInstallationContext = {
      ...
      serverType: server.serverType,
      phpBinary,
      defaultNewSiteLevel: monitoringConfig.defaultNewSiteLevel,
      // The site is already known, so treat it as such and never report it as "new".
      knownInstallationPaths: new Set([wpDir]),
      hadCompletedScan: false,
      newSitesFound: [],
      sendEvent
    }

    const success = await scanInstallation(wpDir, ssh, scanContext)
    const result: ServerScanResult = { success: success ? 1 : 0, failed: success ? 0 : 1 }

    try {
      await associateCvesToSites()
    } catch (error) {
      console.error(`[serverScan] CVE association failed for site ${siteId}:`, error)
    }

    sendEvent('progress', `Processed ${wpDir}`, { total: 1, current: 1, ...result })
    sendEvent(
      'complete',
      success ? 'Site scan completed' : 'Site scan failed',
      result
    )
    return result
  } catch (err: any) {
    sendEvent('error', `Site scan error: ${err.message}`)
    sendEvent('complete', 'Site scan failed', { success: 0, failed: 1 })
    return { success: 0, failed: 1 }
  } finally {
    ssh.disconnect()
  }
